...
# create Netlist
def createnetlist(wn,ln,nf,m,vgs_start,vgs_stop,vgs_step,vbs_start,vbs_stop,vbs_step,vdlin,temp,terminals,lib_path,corner,modelname):
    ...
    with open(filename, 'w') as f:
        ...
        f.write('.param GG=0 \n')
        f.write('.param vbb=' + str(vbs_start) +  '\n')
        ...
        f.write('.DC GG ' + str(vgs_start) + ' ' + str(vgs_stop) +
                ' ' + str(vgs_step) + ' sweep ' + 'vbb ' + str(vbs_start) +
                ' ' + str(vbs_stop) + ' ' + str(vbs_step) +   '\n')
    
        f.write('.temp ' + str(temp) + '\n')
        f.write('.meas dc vtlin find par(\'GG\') when par(\'(abs(i(vd1)))\') = \'1e-7*(wn/ln)\' \n\n' )
        f.write('.meas dc Idsat_abs find par(\'(abs(i(vd)))\') when par(\'GG\')=VGmax \n\n')
        f.write('.meas dc Idsat_nom find par(\'(abs(i(vd))/(wn))\') when par(\'GG\')=VGmax \n\n')
...
# create Netlist
def createnetlist_pointmodel(wn, ln, nf1, m1, vgsStart, vgsMax, vgsStep, vdsStart, vdsMax, vdsStep,vdSat, vdLin, temp, terminals,
                  lib_path, corner, modelname):
    filename = "netlist_point.sp"
    ...
    with open(filename, 'w') as f:
        ...
        f.write('.param wn=' + str(wn) + '\n')
        f.write('.param ln=' + str(ln) + '\n')
        f.write('.param m1=' + str(m1) + '\n')
        f.write('.param nf1=' + str(nf1) + '\n')
        f.write('\n')
